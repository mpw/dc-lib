...
//  Last changed:   $Date: 2012/09/25 15:22:48 $
...
/*int gNumAct[N*N+100];
int gNumFinalAct[N*N+100];
double gMoveTimes[N*N+100];

double gMousePressTimes[N*N+100];
double gSolveTimes[N*N+100];

int gNumMoves=0;
int gNumEvt=1;

struct timeval tvBegin, tvEnd;
struct timeval tvMousePressBegin, tvMousePressEnd;
*/
...
void MyThread::run()
{
    ...
    g_rec_file=fopen ("ROthello2_clickstream.txt", "r");
...
void MyWidget::record ()
{
    ...
    g_rec_file=fopen ("ROthello2_clickstream.txt", "w");
...
void GeneralConstraint (void *data)
{
	...
	if (theColor==1)
	{
		...
		{
			//creates ellipse...
			/*w[i][j]=new Q3CanvasEllipse (25, 25, myCanvas);
	...
	//updates canvas...
	//myCanvas->update (); -> MOVED TO MOUSE HANDLER...
}

 ...
 *  Last changed:   $Date: 2011/04/03 15:20:49 $
...
char* do_updates(test_t* test)  {
    ...
    LGraph_ForAllEdges (test->graph, edge) {
        ...
        if (new_weight < weight) {
            ddsp_decrease(test->d, edge, new_weight);
            test->num_updates++;
            ...
            if (test->check) {
                ...
                // compares distances...
                printf("***** compare distances:\n");
                LGraph_ForAllNodes (test->graph, node) {
        
                    ui4 ok_dist  = LNodeInfo_UI4At(dist, node);
                    ui4 ddsp_dist = ddsp_get_dist(test->d, node);
                
                    if (ddsp_dist != ok_dist) {
                        printf("ok=%lu - ddsp=%lu\n", ok_dist, ddsp_dist);
    ...
    #if DDSP_STAT
    ddssp_stat stat;
    ddsp_get_stat(test->d, &stat);
    printf(">>> Number of scans: %lu\n", stat.num_scans );
    printf(">>> Average number of scans per update: %f\n",
        (double)stat.num_scans / stat.num_updates);
    #endif

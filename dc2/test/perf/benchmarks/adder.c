...
struct adder_s {
    ...
    dc_cons*  cons_id;  // constraint associated with output node
...
static adder_t* adder_create(rnode** in, int* sum, size_t refcount);
...
static adder_t* adder_create(rnode** in, int* sum, size_t refcount) {
    ...
    m->cons_id = dc_new_cons((dc_cons_f)adder_cons, m, NULL);
    ...
    if (m->next != next) {
        ...
        if (next != NULL) {
            if (next->refcount++ == 0) {
                ...
                dc_schedule_final(next->cons_id, NULL);
        ...
        if (m->next != NULL) {
            ...
            if (--m->next->refcount == 0) {
                dc_schedule_final(m->next->cons_id, (dc_cons_f)adder_final);
...
// adder_new
// ---------------------------------------------------------------------
adder_t* adder_new(rnode** in, int* sum) {
